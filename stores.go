...
func (r *oauthProxy) StoreRefreshToken(tck *ticket, value string, expiration time.Duration) error {
	return r.store.Set(tck.id, value, expiration)
}

// Get retrieves a token from the store, the key we are using here is the access token
func (r *oauthProxy) GetRefreshToken(tck *ticket) (string, error) {
	// step: the key is the access token
	v, err := r.store.Get(tck.id)
	if err != nil {
		return v, err
	}
	if v == "" {
...
func (r *oauthProxy) DeleteRefreshToken(tck *ticket) error {
	if err := r.store.Delete(tck.id); err != nil {
